...
Examples: 
* `findtag friends`<br>
  Returns `John Doe` but not `Amy`, assuming `John Doe` has tag `friends` while `Amy` has tag `Friends`
* `find friends owesMoney`<br>
  Returns Any person having tags `friends` or `owesMoney`
